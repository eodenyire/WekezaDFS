...
# --- ATM Cash Offloading UI ---
def offload_atm_ui(officer_info):
    ...
    # Input: Cash amount being offloaded
    amount = st.number_input(
        "Enter Cash Amount to Offload (KES)",
        min_value=0.0,
        step=100.0,
        format="%.2f"
    )
    ...
    if st.button("Offload Cash from ATM"):
        # Validate inputs
        if amount <= 0:
        ...
        # Supervisor approval for large cash amounts
        if requires_supervisor_approval(officer_info["officer_id"], amount):
            st.warning("Supervisor approval required for offloading this ATM amount.")
        ...
        # Call backend API
        try:
            response = post_request("/cash-office/atm-offload", payload)
            if response.get("status") == "success":
                show_cash_receipt("ATM Cash Offloading Receipt", {
                    "ATM ID": atm_id,
                    "Offloaded Amount": amount,
                    "Denominations": denominations,
                    "Processed By": officer_info["officer_id"],
                    "Branch": officer_info["branch_code"],
                    "Timestamp": payload["timestamp"]
                })
                st.success(f"✅ Successfully offloaded KES {amount:,.2f} from ATM {atm_id}.")
            else:
                st.error(f"Failed to offload ATM: {response.get('detail')}")
