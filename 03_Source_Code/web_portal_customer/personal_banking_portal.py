...
def process_loan_payment(user_data, loan, payment_amount):
    ...
    try:
        ...
        cursor.execute("UPDATE accounts SET balance = balance - %s WHERE user_id = %s", 
                      (payment_amount, user_data['user_id']))
        
        # For active loans, use the loan_id directly, otherwise look up by application_id
        if loan.get('loan_id'):
            # This is an active loan from loan_accounts table
            loan_id = loan['loan_id']
            
            # Record loan payment in loan_payments table
            payment_id = f"LP{datetime.now().strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:4].upper()}"
            
            # Calculate principal and interest portions (simplified calculation)
            ...
            """, (payment_id, loan_id, payment_amount, principal_amount, 
                  interest_amount, datetime.now().date(), 'Account Balance', f"USER_{user_data['user_id']}"))
            
            # Update outstanding balance in loan_accounts
            cursor.execute("""
                UPDATE loan_accounts SET outstanding_balance = GREATEST(0, outstanding_balance - %s),
                                       updated_at = %s
                WHERE loan_id = %s
            """, (principal_amount, datetime.now(), loan_id))
            
            # Get updated balance
            cursor.execute("SELECT outstanding_balance FROM loan_accounts WHERE loan_id = %s", (loan_id,))
            updated_loan = cursor.fetchone()
            new_balance = float(updated_loan['outstanding_balance'])
            
            # Check if loan is fully paid
            if new_balance <= 0:
                cursor.execute("""
                    UPDATE loan_accounts SET status = 'CLOSED', outstanding_balance = 0, updated_at = %s 
                    WHERE loan_id = %s
                """, (datetime.now(), loan_id))
        else:
            # This is a loan from loan_applications, try to find corresponding loan_account
            cursor.execute("SELECT * FROM loan_accounts WHERE application_id = %s", (loan['application_id'],))
            ...
            if loan_account:
                ...
                # Convert to float to handle Decimal types from database
                payment_float = float(payment_amount)
                principal_amount = payment_float * 0.8
                interest_amount = payment_float * 0.2
                ...
                # No loan account found, just record as a general payment
                new_balance = float(loan.get('outstanding_balance', loan['loan_amount'])) - payment_amount
        ...
        """, (payment_amount, ref_code, f"Loan payment for {loan.get('loan_type', 'Loan')} - {loan.get('loan_id', loan['application_id'])}", 
        ...
        if new_balance <= 0:
            st.success(f"🎉 Loan fully paid! Thank you for your business.")
            st.balloons()
        else:
            st.success(f"✅ Payment successful! Remaining balance: KES {new_balance:,.2f}")
