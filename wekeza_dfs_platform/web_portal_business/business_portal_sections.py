...
def render_single_payments_section(user_data):
    ...
    with st.form("single_payment_form"):
        ...
        if submitted and payment_amount > 0:
            ...
            if payment_type == "Internal Transfer (Same Bank)":
                st.session_state['recipient_account'] = recipient_account
                st.session_state['recipient_name'] = recipient_name
            elif payment_type == "Mobile Money Transfer":
                st.session_state['recipient_phone'] = recipient_phone
                st.session_state['recipient_name'] = recipient_name
            elif payment_type in ["External Transfer (Other Banks)", "RTGS Transfer", "EFT Transfer"]:
                st.session_state['recipient_account'] = recipient_account
                st.session_state['recipient_name'] = recipient_name
                st.session_state['recipient_bank'] = recipient_bank
...
def process_business_payment(user_data, payment_type, amount, fee, requires_approval):
    """Process business payment through maker-checker system"""
    try:
        ...
        business_account = user_data.get('account_number', 'BIZ1000014')  # Default for demo
        ...
        if payment_type == "Internal Transfer (Same Bank)":
            ...
            operation_type = 'BANK_TRANSFER'  # Default for international transfers
        
        # Get business details for maker info
        business_info = {
            'staff_code': f"BIZ_{user_data.get('business_id', '14')}",  # Use staff_code instead of teller_id
            'full_name': f"Business User - {user_data.get('business_name', 'Business Customer')}",
            'branch_code': 'BIZ001',  # Business portal branch
            'role': 'business_user'
        }
        
        # Check authorization requirements - business payments always require approval
        auth_check = check_authorization_thresholds(operation_type, amount, 'business_user')
        
        # Build operation data based on payment type
        total_amount = float(amount) + float(fee)
        
        if payment_type == "Internal Transfer (Same Bank)":
            # Get recipient account from form (we'll need to pass this)
            recipient_account = st.session_state.get('recipient_account', 'ACC1000023')  # Default for demo
            operation_data = {
                "sender_account": business_account,
                "recipient_account": recipient_account,
                "amount": float(amount),
                "fee": float(fee),
                "total_amount": total_amount,
                "reference": f"Business payment - {payment_type}",
                "transfer_type": "Internal Business Transfer",
                "transaction_date": datetime.now().isoformat(),
                "business_id": user_data.get('business_id', '14'),
                "branch_code": "BIZ001"
            }
        elif payment_type == "Mobile Money Transfer":
            operation_data = {
                "account_number": business_account,
                "recipient_phone": st.session_state.get('recipient_phone', '254700000000'),
                "recipient_name": st.session_state.get('recipient_name', 'Mobile Recipient'),
                "amount": float(amount),
                "fee": float(fee),
                "total_amount": total_amount,
                "reason": f"Business payment - {payment_type}",
                "provider": "M-PESA",
                "transaction_date": datetime.now().isoformat(),
                "business_id": user_data.get('business_id', '14'),
                "branch_code": "BIZ001"
            ...
            # External transfers and others
            operation_data = {
                "sender_account": business_account,
                "recipient_account": st.session_state.get('recipient_account', 'EXT123456'),
                "recipient_bank": st.session_state.get('recipient_bank', 'External Bank'),
                "amount": float(amount),
                "fee": float(fee),
                "total_amount": total_amount,
                "reference": f"Business payment - {payment_type}",
                "transfer_type": f"Business {payment_type}",
                "transaction_date": datetime.now().isoformat(),
                "business_id": user_data.get('business_id', '14'),
                "branch_code": "BIZ001"
            }
        ...
        result = submit_to_authorization_queue(
            ...
            priority='HIGH'  # Business payments high priority
        )
        ...
        if result['success']:
            ...
            st.warning("⚠️ **Supervisor approval required** - Payment will be processed after approval")
            
            # Display authorization receipt
            st.markdown("### 🧾 Business Payment Authorization Receipt")
            st.code(f"""
WEKEZA BANK - BUSINESS PAYMENT AUTHORIZATION
Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
Queue ID: {result['queue_id']}
Business: {user_data.get('business_name', 'Business Customer')}
...
3. Recipient will be credited after approval
            """)
        else:
            st.error(f"❌ Payment submission failed: {result['error']}")
