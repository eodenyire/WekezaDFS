...
# 3. ENHANCED BUSINESS MANAGEMENT
    ...
    with tab2:
        ...
        # Note: Size filter removed as employee_count column doesn't exist
        ...
        if not df_businesses.empty:
            ...
            col3.metric("Average Balance", f"KES {df_businesses['balance'].mean():,.2f}")
            col4.metric("Active Businesses", len(df_businesses[df_businesses['account_status'] == 'ACTIVE']))
        else:
    ...
    with tab4:
        ...
        with col2:
            ...
            df_sector_dist = pd.read_sql("""
                SELECT sector, COUNT(*) as business_count
                FROM businesses 
                GROUP BY sector
        ...
        with col1:
            ...
            df_top_businesses = pd.read_sql("""
                SELECT b.business_name, a.balance, b.sector
                FROM businesses b
        ...
        with col2:
            ...
            # Business registration trend (since employee_count doesn't exist)
            df_registration_trend = pd.read_sql("""
                SELECT 
            ...
            if not df_registration_trend.empty:
                st.dataframe(df_registration_trend, use_container_width=True)
            else:
                st.info("No registration trend data")
