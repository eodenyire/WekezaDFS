...
def create_fixed_deposit(user_data, term_info, amount, auto_renew):
    """Create a fixed deposit through authorization queue (maker-checker system)"""
    try:
        ...
        term_months = int(term_info['term'].split()[0])
        interest_earned = (amount * term_info['rate'] / 100) * (term_months / 12)
        ...
        priority = "URGENT" if amount > 1000000 else "HIGH" if amount > 500000 else "MEDIUM"
        
        # Submit to authorization queue
        result = submit_to_authorization_queue(
            operation_type='FIXED_DEPOSIT_CREATION',
        ...
        if result.get('success'):
            st.success(f"✅ Fixed deposit creation submitted for supervisor approval!")
            st.info(f"📋 Queue ID: {result['queue_id']}")
            st.info(f"💰 Deposit Amount: KES {amount:,.2f}")
            st.info(f"📈 Interest Rate: {term_info['rate']}% p.a.")
            st.info(f"⏰ Term: {term_info['term']}")
            st.info(f"💎 Maturity Amount: KES {maturity_amount:,.2f}")
            st.warning("⚠️ **Important:** Your fixed deposit creation requires supervisor approval before processing.")
            st.info("🔄 You will be notified once a supervisor reviews your request")
            st.info("⏱️ Typical approval time: 2-4 business hours during banking hours")
            
            # Display authorization receipt
            st.markdown("### 🧾 Fixed Deposit Authorization Receipt")
            st.code(f"""
WEKEZA BANK - FIXED DEPOSIT AUTHORIZATION
...
Principal Amount: KES {amount:,.2f}
Interest Rate: {term_info['rate']}% per annum
Term: {term_info['term']}
Expected Interest: KES {interest_earned:,.2f}
Maturity Amount: KES {maturity_amount:,.2f}
Auto-Renewal: {'Yes' if auto_renew else 'No'}
...
2. Fixed deposit will be created after approval
3. Funds will be locked for the specified term
            """)
            st.balloons()
        else:
            st.error(f"❌ Fixed deposit submission failed: {result.get('error', 'Unknown error')}")
